...
export const Sidenav = ({ isOpen, toggle, children }) => {
  ...
  return (
    <aside
      className={
        isOpen ? `${className.default} ${className.enabled}` : `${className.default} ${className.disabled}`
      }
      ref={ref}
    >
...
/* Sidenav classNames */
const className = {
  default: `left h-screen fixed z-20 top-0 left-0 transition-all ease duration-200`,
