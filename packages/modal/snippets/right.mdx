...
const Modal = ({ children, isOpen, toggle }) => {
  const ref = useRef();
  const position = `mt-12 mx-8 md:flex md:w-96 md:h-full md:m-0 md:right-0 md:mx-0 md:my-0 md:absolute`;
        ...
        <div className="container">
          ...
          <div className="fixed top-0 left-0 z-40 w-full h-full m-0">
            <div
              ...
              aria-modal={true}
            >
              <div className="animate-modal-right relative flex flex-col bg-white pointer-events-auto">
                {children}
              </div>
            </div>
          </div>
