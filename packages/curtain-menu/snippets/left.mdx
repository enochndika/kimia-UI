...
export const CurtainMenuPage = () => {
  ...
  return (
    <>
      <button
        onClick={toggle}
        className="text-white focus:outline-none shadow m-1.5 rounded px-6 py-2 font-medium transition ease-in duration-200 bg-black"
      >
        Open
      </button>
      <Menu isOpen={isOpen}>
...
const Menu = ({ children, isOpen }) => {
  ...
  const inactive = `bg-black w-0 h-full fixed z-10 top-0 left-0 overflow-x-hidden md:overflow-hidden transition-all duration-500 `;
