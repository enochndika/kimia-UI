    ...
    <nav>
      {childrenArray.map((child) => (
        <div
          ...
          className={
            current === child.key
              ? 'text-gray-700  border-b-2 border-indigo-700 -mb-2 px-4 md:px-8 text-sm  py-4 md:py-1 inline-block'
              : '-mb-2 px-4 md:px-8 text-sm  py-4 md:py-1 inline-block border-b'
          }
        >
          {child.props.title}
        </div>
